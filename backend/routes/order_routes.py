...
#             kitchen_ids,
#             "New kitchen order",
#             f"Order {order.order_number} is ready to be claimed and prepared.",
...
#             order,
#             "Order accepted",
#             f"Your order {order.order_number} was accepted. We are waiting for payment confirmation.",
...
#         [order.delivery_agent_id] if order.delivery_agent_id else []
#     )
#     notify_staff(
#         order,
#         proof_reviewer_ids,
...
@role_required(["ADMIN", "SHOP_MANAGER", "DELIVERY_AGENT"])
def get_driver_assigned_orders(driver_id):
    orders = Order.query.filter_by(driver_id=driver_id).filter(Order.delivery_method == "DELIVERY").order_by(Order.created_at.desc()).all()
    return jsonify({"count": len(orders), "orders": [o.to_dict() for o in orders]}), 200
...
def create_sales_agent_order():
    ...
    try:
        ...
        area = None
        # If delivery, area_id is required and must be valid
        ...
        order = Order(
            ...
            payment_status="PENDING",
            subtotal=0,
            delivery_charge=(Decimal(str(area.delivery_charge or 0)) if (area and delivery_method == "DELIVERY") else Decimal("0")),
            discount=Decimal("0"),
            grand_total=0,
            total=0,
            # currency=area.currency
        ...
        delivery_charge = (
        Decimal(str(area.delivery_charge or 0))
        if delivery_method == "DELIVERY"
        else Decimal("0")
        )
